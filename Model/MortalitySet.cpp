...
void MortalitySet::RunEcologicalProcess( GridCell& gcl, Cohort* actingCohort, unsigned currentTimestep, ThreadVariables& partial,
  unsigned currentMonth, MadingleyInitialisation& params, std::vector< std::vector<int> > SortedCohortIndices ) {
    ...
    if( BodyMassIncludingChangeThisTimeStep < 1.e-7 ) {
       ...
       // std::cout << params.mCohortFunctionalGroupDefinitions.GetTraitNames( "Nutrition source" , actingCohort->mFunctionalGroupIndex) << "  is " << actingCohort->mFunctionalGroupIndex << std::endl;
        }
       ...
       // if ( params.mCohortFunctionalGroupDefinitions.GetTraitNames("Nutrition Source" , actingCohort->mFunctionalGroupIndex)== "carnivore"){
      // std::cout << "Death due to starvation for a " << params.mCohortFunctionalGroupDefinitions.GetTraitNames( "Nutrition source" , actingCohort->mFunctionalGroupIndex) << " cohort is " << MortalityRateStarvation << std::endl;
       // }} else {
            MortalityRateStarvation =  0;
       // }
