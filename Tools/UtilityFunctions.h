...
public:
    ...
    /** \brief Find new long and lat given distance travelled
    @param current location, distance travelled in km, bearing travelled at in radians
    @return Location after travel</returns> */
    std::pair<double, double> NewLocationGivenDistance( Location location, double distance, double bearing);
    
    /** \brief Find new long and lat given distance travelled
    @param longitude and latitude
    @return Index of grid cell that contains that coordinate</returns> */
    unsigned FindGridCellGivenLongandLat( double lon, double lat );
    
    /** \brief Find new long and lat given distance travelled
    @param radii in km and locations of centres of circles
    @return fraction of area of first circle that overlaps with second </returns> */
    double FindCircleOverlapGivenCentres( double r1, double r2, Location loc1, Location loc2 );
    
    /** \brief Find distance between given locations
    @param Two separate locations
    @return distance between in km </returns> */
    double DistanceBetweenLocations( Location loc1, Location loc2 );
    
    /** \brief Find bearing between given locations
    @param Two separate locations
    @return inital bearing in radians </returns> */
    double InitialBearingBetweenLocations( Location loc1, Location loc2 );
