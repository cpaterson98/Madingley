    ...
    if( success == true ) {
        if( mMetadata.size( ) > 0 ) {
            ...
            for( unsigned environmentalDataFileIndex = 0; environmentalDataFileIndex < mMetadata.size( ); ++environmentalDataFileIndex ) {
                std::string filePath = "C://Users/science.intern2/Desktop/Data/";
                ...
                ////THESE ARE WHERE THE ERRORS ARE!!!!! - LOOK UP HOW TO ASSEMBLE THIS STRING CORRECTLY 
                /// filePath.append( Convertor::Get( )->ToString( Parameters::Get( )->GetRootDataDirectory( ) )); 
                try {
                    ...
                    e.what( );
                    std::cout << "ERROR > The file Path \"" << filePath << "\" is invalid \n ";
                }
            }
